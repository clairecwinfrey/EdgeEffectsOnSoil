# EdgeEdgebyASV_allSites.R -- ITS/FUNGI
# JULY 29, 2022, 2022 (begun)

# ~~~~CLEAN UP DESCRIPTION~~~~
# This script identifies edge effect FUNGAL (ITS) ASVs using differential abundance analyses
# (DESeq2). 
# (DESeq2). This script:
...
# IT IS NEARLY IDENTICAL TO EDGEEFFECTBYASV_ALLSITES.R
...
try1 <- diffAbundfunct2(ITS_postUbiquity.ps, alpha=0.001)
...
#View(diffAbunDat_tidy) this has 66,871 rows, which is equal to 233 (number of samples) x 287 (number of diff abund ASVs)
##########
...
EU_53N_Soils.ps <- subset_samples(ITS_postUbiqASVsPlus1.ps, EU == "EU_53N")
unique(sample_data(EU_53N_Soils.ps)$EU)
EU_54S_Soils.ps <- subset_samples(ITS_postUbiqASVsPlus1.ps, EU == "EU_54S")
unique(sample_data(EU_54S_Soils.ps)$EU)
EU_8_Soils.ps <- subset_samples(ITS_postUbiqASVsPlus1.ps, EU == "EU_8")
unique(sample_data(EU_8_Soils.ps)$EU)
EU_53S_Soils.ps <- subset_samples(ITS_postUbiqASVsPlus1.ps, EU == "EU_53S")
unique(sample_data(EU_53S_Soils.ps)$EU)
EU_10_Soils.ps <- subset_samples(ITS_postUbiqASVsPlus1.ps, EU == "EU_10")
...
# shows that all now have only 287 ASVs, or the differentially abundant ones
...
length(which(is.na(abundZscores_allEUs)))  #there are 1560 NAs across dataset 
index <- which(is.na(abundZscores_allEUs))
length(index)
dim(abundZscores_allEUs) #287  233
