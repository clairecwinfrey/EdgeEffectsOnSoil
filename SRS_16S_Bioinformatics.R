#' 16S rDNA BIOINFORMATICS PIPELINE FOR SOILS COLLECTED MAY 2021 AT THE SRS
#' This script demultiplexes raw reads from the MiSeq, uses dada2 to trim, denoise, dereplicate, and merge reads, and assigns taxonomy (using SILVA version 132). Finally, it formats the data for use in R.
#' *This is based on a tutorial (version last updated Mar 2, 2020), created by Angela Oliverio and Hannah Holland-Moritz and maintained by other members of the Fierer Lab. The tutorial can be found here: [github.com/fiererlab/dada2_fiererlab/](github.com/fiererlab/dada2_fiererlab/)
#' The original dada2 pipeline can be found here: [https://benjjneb.github.io/dada2/bigdata_paired.html](https://benjjneb.github.io/dada2/bigdata_paired.html)*

#+ setup, include=FALSE
# Setup options for outputing markdown files
...
BiocManager::install("dada2", version = "3.10") #changed from version 3.8 on prompt from error code
...
# CCW Jul 24: okay, so I *think* that I can make this go to my data instead? SHOULD BE ABLE TO, AS THIS HAS THE SAME FILES
data.fp <- "/data/shared/MiSeq/07162021_16s_Claire_soil"

# List all files in shared folder to check path
list.files(data.fp) #Yes, this looks like my data!
...
### CCW Jul 24: CHANGING THIS TO BE MINE
# no map.fp because we don't need it? Caihong said not to put it in shared file?
barcode.fp <- file.path(data.fp, "SRSMay21_16SBarcodes.txt")
I1.fp <- file.path(data.fp, "Undetermined_S0_L001_I1_001.fastq.gz") 
R1.fp <- file.path(data.fp, "Undetermined_S0_L001_R1_001.fastq.gz") 
R2.fp <- file.path(data.fp, "Undetermined_S0_L001_R2_001.fastq.gz") 

# Changed to my project! :)
project.fp <- "/data/winfreyc/SRS_May_2021" # CHANGE ME to project directory; don't append with a "/"

# Set up names of sub directories to stay organized
## CCWJul24: So this creates all of these sub-directories? THESE ARE IN MY SRS_May_2021 FOLDER!
preprocess.fp <- file.path(project.fp, "01_preprocess") #INDENTATION MAKES SUBFOLDERS
    demultiplex.fp <- file.path(preprocess.fp, "demultiplexed")
    filtN.fp <- file.path(preprocess.fp, "filtN") # THIS WAS CREATED ON LINE 249
    trimmed.fp <- file.path(preprocess.fp, "trimmed")
filter.fp <- file.path(project.fp, "02_filter") #WHY ISNT THIS SHOWING UP?
table.fp <- file.path(project.fp, "03_tabletax") #WHY ISNT THIS SHOWING UP?
...
list.files(demultiplex.fp) #CCW: how many files should I expect? I have 812 files for 270 samples (samples plus controls)
# WHY ARE ALL OF THESE UNASSIGNED 
...
#' ## CCW July 23: What are these and how do you tell which is which?
...
#'CCW, JULY 23RD: SO IF NO PRIMERS THIS IS GOOD?
...
## SO this trims off the first 10 from the forward, and the first 20 from the reverse

# look at how many reads were kept
head(filt_out)
# I added
dim(filt_out) #269 samples were kept out of 270 
...
# 1        95.86%           98.55%         98.48%        99.68%

#' Plot the quality of the filtered fastq files.
# figure out which samples, if any, have been filtered out CCW: OUT OF THE RANDOM ONES
...
## I added this below to look at how many
length(remaining_samplesF) # ALL OF THE RANDOM SAMPLES WERE KEPT
length(remaining_samplesR)
...
errF <- learnErrors(filtFs, nbases = 1e8, multithread = TRUE, randomize = TRUE) #23 samples. Why/how did it choose this amount? 
...
# this is commented out because I did pooled!
...
# Read in RDS (r data object that we made earlier)
...
tax <- assignTaxonomy(seqtab.nochim, "/db_files/dada2/silva_nr_v132_train_set.fa", tryRC = TRUE,
                      multithread=TRUE)
summary(tax) #no species level here...
# Default is c("Kingdom", "Phylum", "Class", "Order", "Family", "Genus", "Species"). 
# The taxonomic levels being assigned. Truncates if deeper levels not present in training fasta.
# So, it seems that likely that deeper levels not present in training fasta
...
#'
#'############# JUL 27, 2021-- THIS IS WHERE I STOPPED AND DOWNLOADED FILES #############
#'
