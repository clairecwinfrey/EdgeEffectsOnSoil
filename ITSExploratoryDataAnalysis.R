...
##################################################################
# Check for anything non-fungal and remove if present-- STOP HERE 1:19 JULY 30TH
##################################################################

# Get taxonomy table out of phyloseq:
ITS_taxTable <- taxtable_outta_ps(SRS_ITS_raw.ps)
#View(ITS_taxTable)
#-- STOP HERE 1:19 JULY 30TH
...
dim(SRS_ITS_raw.ps)[1] - dim(tax_noeuksorNAs)[1]
...
all_Taxa <- taxa_names(SRS_ITS_raw.ps) #get all tax names in original, uncleaned dataset
ASVstoKeep <- all_Taxa[!(all_Taxa %in% bad_ASVs)]
length(ASVstoKeep) #35000
noeuksorNAs_ps <- prune_taxa(ASVstoKeep, SRS_ITS_raw.ps) #new phyloseq object with just the stuff we want!
